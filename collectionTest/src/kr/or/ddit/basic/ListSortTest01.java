 ...
 * 	  구현하는 인터페이스이다(내부 정렬기준 구현)
 * 
 * - Comparator는 외부에 별도로 정렬 기준을 구현하고 싶을 때 사용하는 인터페이스이다(외부 정렬기준 구현)
 * 
...
// 정렬 방식을 정해주는 class 만들기(외부 정렬 기준 class만들기)
class Desc implements Comparator<String> {

	// compare메서드의 반환값의 역할
	// - 반환값이 0 -> 두 값이 같다
	// - 반환값이 양수 -> 두 값의 순서를 변경한다
	// - 반환값이 음수 -> 두 값의 순서를 변경하지 않는다
	
	// ex) 오름차순일 경우 -> 앞의 값이 크면 양수, 같으면 0, 뒤의 값이 크면 음수가 반환되도록 구현
	@Override
